...
def _gateway_payment_id(order: dict) -> str:
    return order.get('gatewayTransactionId') or order.get('gatewayDepositId') or '-'


def _qr_image_to_file(qr_image: str) -> discord.File | None:
    """Decode Tokoshopp's data-URI QR image into a Discord attachment."""
    if not qr_image:
        return None
    try:
        if qr_image.startswith('data:image/') and ';base64,' in qr_image:
            meta, encoded = qr_image.split(';base64,', 1)
            mime = meta[5:]
            ...
            if ext not in {'png', 'jpg', 'gif', 'webp'}:
                ext = 'png'
            return discord.File(io.BytesIO(base64.b64decode(encoded)), filename=f'payment_qr.{ext}')
        if _URL_RE.match(qr_image):
            return None
...
def _is_free_product(price) -> bool:
    """A product is free only when its stored/input price is exactly numeric zero."""
...
async def resume_pending_payments(bot: commands.Bot):
    ...
    a restart:
    1. The poll loop that checks Tokoshopp and auto-delivers -- lives only
...
class ProductCog(commands.Cog):
    ...
    async def buy(self, interaction: discord.Interaction):
        ...
        amount = item.get('lineTotal') if item.get('lineTotal') is not None else _parse_price_local(item.get('price'))
        is_free = _is_free_product(product.get('price'))

        if is_free:
            if user_owns_product(product, str(interaction.user.id)):
                return await interaction.followup.send(f"You already own **{product['name']}**.")
            ...
            if file_link:
                try:
                    await buyer.send(**build_product_delivery_dm(product, file_link=file_link))
                except discord.HTTPException as err:
                    print(f'[product buy] could not DM free product to {buyer.id}: {err!r}')

            await _post_auto_testimony(
                interaction.client, str(interaction.guild_id), str(interaction.user.id), [product['name']]
            )
            await log_command_activity(
                interaction, subcommand='buy', success=True,
                fields={'discordUser': interaction.user, 'productId': product['id'], 'productName': product['name']},
                note=f"Free product {product['id']} delivered without payment.",
            )
            return await interaction.followup.send(
                f"✅ **{product['name']}** is free and has been delivered to your DM."
            )

        if amount <= 0:
            return await interaction.followup.send('The product price must be exactly `0` to be free.')
        ...
        try:
            ...
            await log_command_activity(
                ...
                fields={'discordUser': interaction.user, 'productId': product['id'], 'productName': product['name']},
                note='Tokoshopp payment/create failed.',
            )
        ...
        embed.add_field(name='Transaction ID', value=f"`{_gateway_payment_id(order)}`", inline=True)
        embed.set_footer(text='Scan QRIS dengan e-wallet/banking yang mendukung. Bayar PERSIS nominal di atas. Kadaluarsa dalam ~30 menit.')

        qr_image = order.get('qrImageUrl') or ''
        qr_file = _qr_image_to_file(qr_image)
        if qr_file:
            embed.set_image(url=f'attachment://{qr_file.filename}')
        elif _URL_RE.match(qr_image):
        ...
        message = await interaction.followup.send(
            embed=embed, view=view, file=qr_file, wait=True
        )
    ...
    @product_group.command(name='forcecheck', description='Force-check this ticket\'s pending payment against Tokoshopp directly')
    async def forcecheck(self, interaction: discord.Interaction):
        ...
        try:
            ...
            return await interaction.followup.send(
                f"Gagal mengecek status ke Tokoshopp. Transaction ID: `{deposit_id}`. Coba lagi sebentar lagi."
            )

        if not confirmed:
            return await interaction.followup.send(
                f"Belum terbayar menurut Tokoshopp. Transaction ID: `{deposit_id}`, Order ID: `{order['id']}`."
            )
        ...
        await interaction.followup.send(
            f"✅ Pembayaran terkonfirmasi (Transaction ID: `{deposit_id}`). Produk sudah dikirim ke DM pembeli."
        )
